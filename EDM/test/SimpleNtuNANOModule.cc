...
class SimpleNtuNANOModule: public SimpleNtuple, // ntuple definition,
                           public SimpleFill,   // assign values to data,
                           public NANOTreeWriter { // create branches

 public:

  explicit SimpleNtuNANOModule( const edm::ParameterSet& ps ) {
  ...
  bool fill( const edm::Event& ev, const edm::EventSetup& es ) override {
    ...
    setData( id.run(),         // assign values to data   (in SimpleFill  )
             id.event() );
    return n_arr > nCut;       // in NANOAOD the output ntuple must be filled
                               // for all events; the return value (true/false)
                               // can be used in an EDFilter to choose
                               // if the event is to be passed to following
                               // modules, or not (see cfg_ntuNANO.py)
  }
...
DEFINE_FWK_MODULE( SimpleNtuNANOModule );
